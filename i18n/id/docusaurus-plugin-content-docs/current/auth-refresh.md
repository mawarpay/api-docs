...
Gunakan endpoint ini ketika access token dari [Login](./login-v1) sudah kedaluwarsa atau mendekati kedaluwarsa.
...
Bentuk respons sama dengan [Login](./login-v1): `code`, `message`, dan `data` berisi `accessToken`, `tokenType`, `expiresIn`, serta `refreshToken`.
...
- Jika berulang kali mendapat `401` dari refresh, minta pengguna untuk login ulang melalui [Login](./login-v1).
