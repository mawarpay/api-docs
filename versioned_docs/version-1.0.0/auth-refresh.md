...
Use this endpoint when an access token from [Login](./login-v1) is expired or near expiry.
...
Response shape matches [Login](./login-v1): `code`, `message`, and `data` with `accessToken`, `tokenType`, `expiresIn`, and `refreshToken`.
...
- On repeated `401` from refresh, require the user to log in again via [Login](./login-v1).
